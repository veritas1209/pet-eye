...
void printStatus() {
    ...
    if (now - lastStatus >= STATUS_PRINT_INTERVAL) {
        ...
        // HTTP 스트리밍 상태 출력
        if (streaming_config.streaming_active) {
            float currentFPS = 1000.0 / streaming_config.frame_interval;
            Serial.printf("     📡 HTTP 스트리밍: %.1f FPS | 메모리: %dKB\n", 
                         currentFPS, ESP.getFreeHeap() / 1024);
        } else {
            Serial.printf("     ❌ 스트리밍 중지됨 | 메모리: %dKB\n", ESP.getFreeHeap() / 1024);
        }
...
void setup() {
    ...
    if (camera_ok) {
        ...
        Serial.println("❌ 카메라 초기화 실패 - 계속 진행");
    }
    ...
    if (isWiFiConnected()) {
        ...
        // 6. HTTP 스트리밍 시작
        if (startHTTPStreaming()) {
            Serial.println("✅ 🎥 HTTP 스트리밍 시작!");
            Serial.println("     서버에서 펫 영상을 확인할 수 있습니다.");
        } else {
            Serial.println("⚠️  HTTP 스트리밍 실패 - 센서 모드로 동작");
        }
    } else {
...
void loop() {
    ...
    if (now - lastBackupSend >= 60000) { // 1분마다
        if (isWiFiConnected() && sensors.data_ready) {
            // HTTP로도 센서 데이터 백업 전송 (http_client.h에서 정의됨)
            sendSensorData();
            lastBackupSend = now;
        }
    }
